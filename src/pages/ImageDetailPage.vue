  ...
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-gray-50 rounded-lg overflow-hidden mb-8">
      <div class="relative">
        ...
        <div class="absolute bottom-4 right-4 flex space-x-2">
          ...
          >
            <i
              class="fas fa-bookmark w-5 h-5"
              :style="bookmarked ? 'fill: currentColor;' : ''"
            ></i>
          </button>
          <button
            class="p-2 rounded-full bg-black/50 text-white hover:bg-black/60"
          >
          ...
          <div class="relative">
            ...
            <div
              ...
              class="absolute bottom-full right-0 mb-2 w-64 bg-white rounded-lg shadow-lg overflow-hidden"
            >
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2">
        <h1 class="text-3xl font-bold mb-4">{{ image2.title }}</h1>
        <p class="text-gray-600 mb-8">{{ image2.description }}</p>
      ...
      <div>
        <div class="bg-gray-50 rounded-lg p-6">
          <div class="flex items-center mb-4">
            <img
              :src="image2.account.avatarUrl"
              :alt="image2.account.name"
              class="w-16 h-16 rounded-full mr-4"
            />
            <div>
              <h3 class="font-semibold text-lg">
                {{ image2.account.firstName }} {{ image2.account.lastName }}
              </h3>
              <p class="text-gray-500 break-words">
                @{{ image2.account.username }}
              </p>
            </div>
          </div>
          ...
          <div class="flex justify-between mb-6 text-sm text-gray-500">
            <div>
              <div class="font-semibold text-gray-900">
                {{ image.photographer.photos }}
              </div>
              <div>Photos</div>
            </div>
            <div>
              <div class="font-semibold text-gray-900">
                {{ image.photographer.followers }}
              </div>
              <div>Followers</div>
            </div>
            <div>
              <div class="font-semibold text-gray-900">
                {{ (image.downloads / 1000).toFixed(1) }}K
              </div>
              <div>Downloads</div>
            </div>
          </div>
...
export default {
  created() {
    ...
    if (!isNaN(id)) {
      ...
      console.warn("ID không hợp lệ:", this.$route.params.id);
    }
